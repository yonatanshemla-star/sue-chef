          ...
          {results.map((res, idx) => (
            <div key={idx} className={`p-8 rounded-[32px] border border-white/20 premium-glass shadow-xl animate-in slide-in-from-bottom-8 duration-700 delay-${idx*100}`}>
              <div className="flex items-start gap-4 mb-4">
                ...
                <h3 className={`text-2xl font-bold ${res.type === 'tax' ? 'text-amber-600 dark:text-amber-400' : 'text-indigo-600 dark:text-indigo-400'}`}>{res.title}</h3>
              </div>
              <p className="text-slate-600 dark:text-slate-300 leading-loose font-bold text-lg opacity-90 pr-7">{res.description}</p>
            </div>
          ))}
        </div>
        <button onClick={reset} className="mt-10 flex items-center gap-3 px-10 py-5 bg-gradient-to-r from-indigo-600 to-indigo-800 text-white rounded-[24px] font-bold self-center hover:scale-110 hover:-rotate-1 transition-all active:scale-95 shadow-2xl shadow-indigo-500/20">
    ...
    <div className={`flex flex-col gap-10 ${compact ? 'p-0' : 'p-4 md:p-10 max-w-4xl mx-auto'} animate-in fade-in duration-700`}>
      ...
      {!compact && (
        <div className="flex flex-wrap gap-2.5 justify-center">
          {history.map((idx, i) => (
            <span key={i} className="px-4 py-2 rounded-full bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border border-white/20 text-[10px] font-bold text-slate-400 uppercase tracking-widest animate-in fade-in slide-in-from-top-2 duration-500">
      ...
      <div className={`relative p-6 md:p-16 rounded-3xl md:rounded-[48px] premium-glass border border-white/20 shadow-[0_32px_128px_-12px_rgba(0,0,0,0.5)] flex flex-col gap-8 md:gap-12 animate-in fade-in zoom-in-95 duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] text-center`}>
        ...
        <div className="flex items-center justify-between pt-10 mt-4 border-t border-indigo-500/10">
          <button 
            className="flex items-center gap-3 px-8 py-4 rounded-[20px] font-bold text-sm uppercase tracking-widest bg-slate-200/50 dark:bg-slate-800/50 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all disabled:opacity-0 disabled:pointer-events-none transition-all duration-500 hover:-translate-x-2" 
            onClick={handleBack}
          ...
          <button 
            className="flex items-center gap-4 px-12 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[28px] font-bold text-lg shadow-2xl hover:scale-110 hover:rotate-1 active:scale-95 transition-all duration-300" 
            onClick={() => handleNext()}
          >
