          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-slate-300 flex items-center gap-3">
                Sue-Chef 
                <span className="text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-full border border-emerald-500/20 font-bold tracking-widest uppercase">עו"ד</span>
              </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border dark:border-slate-800 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1">
            <div className="flex items-center gap-4 mb-4">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">לקוחות חתומים</p>
            </div>
            <p className="text-4xl font-bold text-emerald-600 dark:text-emerald-400">{leads.length}</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border dark:border-slate-800 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1">
            <div className="flex items-center gap-4 mb-4">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">תיקים פעילים</p>
            </div>
            <p className="text-4xl font-bold text-blue-600 dark:text-blue-400">{activeCases}</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border dark:border-slate-800 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1">
            <div className="flex items-center gap-4 mb-4">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">סה"כ רווחים</p>
            </div>
            <p className="text-4xl font-bold text-amber-600 dark:text-amber-400">₪{totalProfit.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-32 text-slate-400">
              ...
              <p className="font-bold">טוען לקוחות חתומים...</p>
            </div>
          ) : filteredLeads.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-slate-400">
              <Users size={64} className="mb-6 opacity-10" />
              <p className="font-bold text-xl opacity-50">אין לקוחות חתומים להצגה</p>
            </div>
          ) : (
            filteredLeads.map((lead, idx) => (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 bg-gradient-to-br from-emerald-100 to-teal-100 dark:from-emerald-900/30 dark:to-teal-900/30 rounded-2xl flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-bold text-xl">
                      {lead.clientName?.charAt(0) || '?'}
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">{lead.clientName || 'לקוח'}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    {lead.profit ? (
                      <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-4 py-1.5 rounded-full">
                  ...
                  <div className="px-6 pb-6 border-t dark:border-slate-800 pt-5 animate-in fade-in slide-in-from-top-2 duration-300 space-y-5">
                    ...
                    {(lead.generalNotes || lead.liveCallNotes) && (
                      <div className="bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl p-5 border border-indigo-100/50 dark:border-indigo-900/30">
                        <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-2 flex items-center gap-2"><FileText size={12} /> הערות מסדר הליד</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {/* Case Status */}
                      <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">סטטוס תיק</label>
                      ...
                      {/* Profit */}
                      <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">רווח מהלקוח (₪)</label>
                        <input
                          type="number"
                          value={lead.profit || ''}
                      ...
                      {/* Phone */}
                      <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">טלפון</label>
                    ...
                    {/* Lawyer Notes */}
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">הערות עו"ד — מעקב התקדמות</label>
                      <textarea
      ...
      {showSwitchModal && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-xl">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm p-10 rounded-[48px] shadow-2xl border dark:border-white/5" dir="rtl">
            ...
            <h3 className="text-2xl font-bold mb-2 text-center">חזרה לניהול CRM</h3>
            ...
            <form onSubmit={handleSwitchRole} className="space-y-4">
              <input type="password" value={switchPassword} onChange={e => setSwitchPassword(e.target.value)} autoFocus placeholder="סיסמה..." className="w-full bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-2xl px-6 py-4 text-center text-lg font-bold outline-none focus:ring-4 focus:ring-blue-500/10 transition-all" dir="ltr" />
              {switchError && <p className="text-xs font-bold text-red-500 text-center">{switchError}</p>}
              <button type="submit" disabled={switchLoading || !switchPassword} className="w-full bg-blue-600 text-white rounded-2xl py-4 font-bold text-lg shadow-lg shadow-blue-500/20 active:scale-95 transition-all flex items-center justify-center">
