...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use signedAt if available, otherwise fall back to created_at
    const signedRes = await sql`
      SELECT count(*) as count
      FROM leads 
      WHERE (data->>'status') = 'חתם'
        AND COALESCE(data->>'signedAt', created_at::text) >= ${weekStartISO}
        AND COALESCE(data->>'signedAt', created_at::text) <= ${weekEndISO}
    `;
