...
export async function syncNewLeadsFromLeadim(): Promise<number> {
  try {
    ...
    // 2. Fetch initial leads page (Page 1) to get actual leads page ViewState
    ...
    let currentHtml = await leadsPageRes.text();
    let currentVs = currentHtml.match(/id="__VIEWSTATE"\s+value="([^"]+)"/)?.[1] || viewstate;
    let currentVsg = currentHtml.match(/id="__VIEWSTATEGENERATOR"\s+value="([^"]+)"/)?.[1] || viewstategen;

    // 3. First set date range to 01/01/2020 00:00 to unlock all historical leads
    ...
    drangeParams.append('__VIEWSTATE', currentVs);
    drangeParams.append('__VIEWSTATEGENERATOR', currentVsg);
    ...
    currentHtml = await drangeRes.text();
    currentVs = currentHtml.match(/id="__VIEWSTATE"\s+value="([^"]+)"/)?.[1] || currentVs;
    currentVsg = currentHtml.match(/id="__VIEWSTATEGENERATOR"\s+value="([^"]+)"/)?.[1] || currentVsg;

    let totalNewLeadsCount = 0;
    let pageNum = 1;
    let keepScanningPages = true;

    // Sequentially scan Page 1, Page 2, Page 3... until we reach a page where ALL leads are already in DB!
    while (keepScanningPages && pageNum <= 15) {
      ...
      // If Page 1 (or any page) had 0 new leads, we are completely caught up!
      if (pageInsertedCount === 0) {
        keepScanningPages = false;
        break;
      }

      // If new leads were found on this page, fetch next page to check if more leads accumulated!
      pageNum++;
      const pageParams = new URLSearchParams();
      pageParams.append('__EVENTTARGET', 'lm$contMain$gv');
      pageParams.append('__EVENTARGUMENT', `Page$${pageNum}`);
      pageParams.append('__VIEWSTATE', currentVs);
      pageParams.append('__VIEWSTATEGENERATOR', currentVsg);

      const pageRes = await fetch(`https://sys.lead.im/a/${accountId}/leads`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Cookie': allCookies,
